...
# Create a calculated column to display % error of predicted v actual data in each row
Combined_Pred_Act <- Combined_Pred_Act %>% mutate(Per_Cent_Predict_Error= ((Predicted_Global_Sales_sum - Global_Sales_sum)/Global_Sales_sum)*100)
...
# Recommend this Model for understanding how changes in regional data may impact changes to global sales in the future.
...
ols_test_normality(mlr_model.lm)
performance::check_model(mlr_model.lm)
# Good fit but poor residuals output
